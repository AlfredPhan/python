#Program no-6 
from tkinter import *
from tkinter.ttk import *  #theme tkinter (ttk)
import googlesearch  # for accessing Google search

def google_search(event=None):
    query = query_txt.get()
    search_results = []
    for search_result in googlesearch.search(query, stop=10, num=1, pause=2):
        print(search_result)
        search_results.append(search_result)
window = Tk()
window.geometry("520x430")
window.title("Web inSite")#embedding the website inside python
...
query_txt.grid(column=1, columnspan=6, row=row_counter,
