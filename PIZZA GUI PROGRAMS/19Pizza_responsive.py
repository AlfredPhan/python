#Program No.-7 Pizza shopping application
from tkinter import *
def clicked():
    pizza_type = pizzas[selected.get()]
    pizza_price = pizza_prices[selected.get()]
    description = pizza_type
    extras_description = ""
    for e in range(len(extra_chosen)):
        if extra_chosen[e].get():
            if extras_description == "":
                extras_description += f" with extra {extras[e]}"
            else:
                extras_description += f", {extras[e]}"
            pizza_price += extra_prices[e]
...
for i in range(len(pizzas)):
    rad = Radiobutton(window, text=f"{pizzas[i]} - £{pizza_prices[i]:.2f}", value=i, variable=selected, command=clicked)
...
for i in range(len(extras)):
    ...
    chk = Checkbutton(window, text=f"{extras[i]} - £{extra_prices[i]:.2f}", var=extra_chosen[i], command=clicked)
